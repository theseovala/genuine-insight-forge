...
SELECT setseed(0.731);

WITH pos AS (
  SELECT row_number() OVER () AS idx, * FROM (VALUES
...
base AS (
  SELECT
    ...
    (ARRAY['Mumbai — Bandra Kurla','Dubai — Marina Walk','London — Soho Square','New York — Midtown East','Singapore — Orchard Road','Sydney — CBD George St'])[1 + floor(random() * 6)::int] AS location_name,
    (ARRAY['google','google','google','google','google','facebook','facebook','trustpilot','trustpilot','yelp','instagram','tripadvisor'])[1 + floor(random() * 12)::int] AS platform,
    (ARRAY['Priya','Daniel','Meera','Lucas','Aisha','Hannah','Marcus','Sofia','Rahul','Elena','Omar','Grace','Liam','Nadia','Isabelle','Jonas','Fatima','Ethan','Ananya','Yuki','Diego','Amara','Noah','Leila','Victor','Sanjay','Clara','Mateo','Zara','Oliver','Karan','Freya','Hugo','Divya','Samuel','Anika','Ruben','Maya','Felix','Tara','Neha','Alice','Bilal','Camille','Dev','Erin','Rosa','Theo','Nina','Arjun'])[1 + floor(random() * 50)::int] AS first_name,
    (ARRAY['Sharma','Okafor','Iyer','Moreau','Khan','Weiss','Bell','Almeida','Verma','Petrova','Haddad','Whitfield','Rahman','Laurent','Berg','Aziz','Clarke','Rao','Hughes','Tanaka','Ramos','Obi','Fischer','Novak','Menon','Jensen','Silva','Malik','Grant','Costa','Batra','Dupont','Nair','Adeyemi','Kapoor','Santos','Lopez','Brandt','Singh','Moore','Joshi','Bennett','Yusuf','Girard','Patel','Walsh','Dias','Frost','Osei','Baptiste'])[1 + floor(random() * 50)::int] AS last_name,
    random() AS roll,
    random() AS roll2,
    random() AS roll3,
    random() AS roll4,
...
sentimented AS (
  SELECT
    c.*,
    CASE
      WHEN c.roll < c.pos_bias THEN 'positive'
      WHEN c.roll < c.pos_bias + (1 - c.pos_bias) * 0.35 THEN 'neutral'
      ELSE 'negative'
    END AS sentiment,
    ((c.days_ago > 3 AND c.roll4 < 0.9) OR (c.days_ago <= 3 AND c.roll4 < 0.25)) AS answered
  FROM classified c
),
withtext AS (
  SELECT
    s.*,
    COALESCE(p.title, n.title, g.title) AS ttl,
    COALESCE(p.body, n.body, g.body) AS bdy,
    COALESCE(p.tags, n.tags, g.tags) AS tgs
  FROM sentimented s
  LEFT JOIN pos p ON s.sentiment = 'positive' AND p.idx = 1 + floor(s.roll2 * 10)::int
  LEFT JOIN neu n ON s.sentiment = 'neutral' AND n.idx = 1 + floor(s.roll2 * 7)::int
  LEFT JOIN neg g ON s.sentiment = 'negative' AND g.idx = 1 + floor(s.roll2 * 8)::int
)
...
SELECT
  ...
  END,
  w.location_name,
  CASE WHEN w.roll3 < 0.55 THEN w.ttl ELSE NULL END,
  w.bdy,
  w.tgs,
  (NOT w.answered) AND w.days_ago < 2,
  CASE WHEN w.answered THEN
    CASE WHEN w.sentiment = 'positive' THEN
      (ARRAY[
    ...
    END
  ELSE NULL END,
  CASE WHEN w.answered THEN now() - (w.days_ago || ' days')::interval + interval '7 hours' ELSE NULL END,
...
WHERE s.cnt >= 4 AND s.pos_pct >= 75;
